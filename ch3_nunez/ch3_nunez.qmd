...
- integration vs isolation
   - Fans sitting in a football stadium
   - **Functional localization**: states in which subsystems act mostly independently of other subsystems
   - **Global coherence**: states in which subsystems combine to form a single intgrated subsystem, perhaps with emergent properties
   - **Binding problem**: issue of how widely didtributed neural activities give rise to a single integrated behavior and consciousness
   ...
   - **complexity science**: study of complex systems composed of many interacting parts. Such systems exhibitglobal behavior that cannot be easily explained (i.e., reduced) in terms of their individual smaller parts in isolation.
...
- **macrocolumn scale** in neocortex is defined by the spatial extent of axon branches E that remain within the cortex (*recurrent/reafferent collaterals*). 

- large **pyramidal** cell (C) is one of 10^5 to 10^6 *neurons* in the macrocolumn. 
...
![](nunez_macrocolumn_3_12.jpg){.absolute top="60" right="-50" height="200"}
![](nunez_3levels_macrocolumn_3_13.jpg){.absolute bottom="-10" left="0" height="250"}
![](TC_contralateral.png){.absolute bottom="-10" right="-50" height="350"}
...
- Basket interneuron (inhibitory)
- Myelinated axon
- **Macro-column** (ipsilateral/contralateral projection): a cortical tissue column defind by the local intracortical spread of individual pyramidal cells; contains about 1 million neurons w/in its diameter of 3mm
...
- Cortical **mini-column**: a cortical tissue column defind by the spatial extent of intracortical **inhibitory connections**; contains about 100 neurons w/in its diameter of 0.3mm
...
- **Epilepsy**: a group of neurological disorders marked by recurrent sensory disturbances, loss of consciousness, and sometimes convulsions. Associated with abnormal EEG activity.
...
#### The Original Social Network & 5.5 Degrees of Separation {.smaller}

![](degrees_6_sep.jpeg){.absolute top="60" right="-60" height="200"}
...
- **Graph theory**: (network) mathematics of nodes (intersection) and edges (links/connections)
...
![](graph.png){.absolute bottom="-10" left="-60" height="400"}
![](graph_perc.png){.absolute bottom="-10" right="-10" height="350"}



::: {.notes}
- Common elements found in complex networks. 
- a, Broad-scale distributions (P) of node degrees (k). 
- b, Clustering with neighbours. 
- c, Community structure: nodes can be allocated to groups for which the density of connections within a group is higher than the density of connections between groups. 
- d, Small- world properties: most nodes are not nearest neighbours but are linked by short paths on the network. 
- e, Phase transitions, such as the percolation transition in which the fractional size S of the largest connected component of the network exhibits a phase transition as the average degree increases.
...
- Several other properties are often associated with small-world networks.
   ...
   - This property is often analyzed by considering the fraction of nodes in the network that have a particular number of connections going into them (the degree distribution of the network). Networks with a greater than expected number of hubs will have a greater fraction of nodes with high degree, and consequently the degree distribution will be enriched at high degree values. 
   - This is known colloquially as a fat-tailed distribution. Graphs of very different topology qualify as small-world networks as long as they satisfy the two definitional requirements above.
